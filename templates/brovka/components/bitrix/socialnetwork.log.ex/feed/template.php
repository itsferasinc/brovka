	...
	foreach ($arResult["Events"] as $arEvent)
	{
		if (!empty($arEvent))
		{
			...
			if(in_array($arEvent["EVENT_ID"], array_merge($blogPostEventIdList, array("blog_comment", "blog_comment_micro"))))
			{
                ...
                if($arEvent['EVENT_ID'] == 'blog_post'){
                    $arBlog = CBlog::GetByOwnerID($arEvent['ENTITY_ID']);
                    $arPost = CBlogPost::GetByID($arEvent['SOURCE_ID']);
                    if($arPost && $arPost['PUBLISH_STATUS'] != "K") {
                        $APPLICATION->IncludeComponent(
                            "bitrix:blog.post",
                            "post",
                            Array(
