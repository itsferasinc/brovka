...
if(count($arResult["POST"])>0)
{
	foreach($arResult["POST"] as $ind => $CurPost)
	{
        ...
        $url = "/blogs/".$arResult['BLOG']['URL']."/".($CurPost['CODE']?$CurPost['CODE']:$CurPost['ID'])."/";
		...
		?>
			...
			BX.viewImageBind(
                ...
                <div class="block forumblogpost" data-id="<?=$CurPost['ID']?>">
                    <p class="h1"><a href="<?=$url?>"><?=trim($CurPost["TITLE"])?></a></p>
                    <div class="info">
                        <i class="flaticon-time"></i>
                        <p class="h4"><?=$CurPost["DATE_PUBLISH_FORMATED"]?></p>
                        ...
                        <p class="h4">Комментариев: <a class="name-link" href="<?=$url?>"><?=$CurPost["NUM_COMMENTS"]?></a></p>
                        <i class="flaticon-eye"></i>
                        <p class="h4">Просмотров: <?=$CurPost["VIEWS"]?></p>
                    ...
                    $themes['PROPERTY_T_BLOGS'] = $CurPost['ID'];?>
                    ...
                    <div class="content-block-footer">
                        <button class="plus-button" onclick="upvote(<?=$CurPost['ID']?>,'blog')">
                            <i class="flaticon-plus"></i>
                            <span><?=(int)$arResult['RATING'][$CurPost['ID']]['TOTAL_POSITIVE_VOTES']?></span>
                        </button>
                        <button class="minus-button" onclick="downvote(<?=$CurPost['ID']?>,'blog')">
                            <i class="flaticon-minus"></i>
                            <span><?=(int)$arResult['RATING'][$CurPost['ID']]['TOTAL_NEGATIVE_VOTES']?></span>
                        </button>
                        <button class="favorite-button" onclick="favorites_add(<?=$CurPost['ID']?>,'blog')" <?if(in_array("b".$CurPost['ID'], $arResult['FAVORITES'])){?>style="display: none"<?}?>>
                            <i class="flaticon-star"></i>
                            <span>Добавить в избранное</span>
                        </button>
                        <button class="favorite-button favorite-active" onclick="favorites_del(<?=$CurPost['ID']?>,'blog')" <?if(in_array("b".$CurPost['ID'], $arResult['FAVORITES'])){?>style="display: block"<?}?>>
                            <i class="flaticon-star"></i>
                            <span>Избранное</span>
                        </button>
                        <div class="share">
                    ...
                    <?if(strLen($CurPost["urlToEdit"])>0 || strLen($CurPost["urlToDelete"])>0){?>
                        ...
                        <?if(strLen($CurPost["urlToEdit"])>0):?>
                            <span class="blog-post-edit-link">
                                <a href="/blogs/edit/<?=$CurPost['ID']?>/<?//=$CurPost["urlToEdit"]?>">
