...
<?
		if ($res["AUTHOR_ID"] > 0):
?>
					<div class="forum-user-name"><?//$res["AUTHOR_NAME"]
						?><?=str_replace(array("#URL#", "#NAME#"), array($res["URL"]["AUTHOR"], ITSGlobal::getUserName($res["AUTHOR_ID"])), $arParams["USER_TMPL"])
					?></div>
<?
...
?>
			...
			<td class="forum-cell-post">
				...
				<div class="forum-post-entry">
                    ...
                    $aUserField = $GLOBALS['USER_FIELD_MANAGER']->GetUserFields(
                        'FORUM_TOPIC',
                        $res['TOPIC_ID']
                    );

                    if(CSite::InGroup(array(1,10)) && $res['NEW_TOPIC'] == "Y"){?>
                        <p><a href="/local/ajax/themes.php?id=<?=$res['TOPIC_ID']?>&type=forum" class="fancybox">Указать темы</a></p>

                        <p><a href="javascript:;" class="button-remove-from-index" onclick="topic_unhide_index(<?=$res['TOPIC_ID']?>,'forum',$(this))" <?if($aUserField['UF_HIDEONINDEX']['VALUE']!=1){?>style="display: none;"<?}?>>Вернуть на главную</a></p>
                        <p><a href="javascript:;" class="button-remove-from-index" onclick="topic_hide_index(<?=$res['TOPIC_ID']?>,'forum',$(this))" <?if($aUserField['UF_HIDEONINDEX']['VALUE']==1){?>style="display: none;"<?}?>>Убрать с главной</a></p>
