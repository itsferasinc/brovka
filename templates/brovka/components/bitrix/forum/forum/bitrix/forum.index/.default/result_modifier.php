...
$hiddenGroups = array(67);

foreach ($arResult['GROUPS'] as $id=>$group){
    if(in_array($group['ID'], $hiddenGroups) && !CSite::InGroup(array(1,10,8)))
        unset($arResult['GROUPS'][$id]);
}
foreach ($arResult["FORUMS"]['GROUPS'] as $id=>$group){
    if(in_array($group['ID'], $hiddenGroups) && !CSite::InGroup(array(1,10,8)))
        unset($arResult["FORUMS"]['GROUPS'][$id]);
}
