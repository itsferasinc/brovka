	...
	<table class="data-table" style="width: 90%">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):
                ...
                ?>
				<tr>
					<td valign="top"><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?></td>
					<td>
						<?
						...
						switch ($INPUT_TYPE):
							...
							case "E":
                                ?>
                                <select class="select2guides" multiple name="PROPERTY[<?=$propertyID?>][]">
                                    <?foreach ($arResult['DICS'][$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]] as $DIC) {
							...
							case "N":
							    if($propertyID == 31){?>
                                    <select class="select2guides" name="PROPERTY[<?= $propertyID ?>][]" multiple>
                                        <?
                                        ...
                                        while ($vars = $db_vars->Fetch()) {
                                            ...
                                            if(strlen($vars['REGION_NAME'])>0 && strlen($vars['CITY_NAME'])<1){?>
		...
		<tfoot>
			<tr>
				<td colspan="2">
                    <input type="hidden" name="PROPERTY[CODE][0]" value="temp">
                    <input type="hidden" name="PROPERTY[NAME][0]" value="temp">
					<input type="submit" class="btn btn-themes" name="iblock_submit" value="Отправить" />
